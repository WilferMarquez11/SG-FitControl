...
export default function PanelEntrenador({ navigation }) {
  const { tema, modoOscuro, toggleTema } = useTheme();
  const [nombreEntrenador, setNombreEntrenador] = useState("Entrenador");
  const [emailEntrenador, setEmailEntrenador] = useState("");
  const [tabActiva, setTabActiva] = useState("PanelEntrenador");
  ...
  useEffect(() => {
    const obtenerNombre = async () => {
      try {
        ...
        if (user) {
          setEmailEntrenador(user.email || "");
          const docSnap = await getDoc(doc(db, "Usuarios", user.uid));
          if (docSnap.exists()) setNombreEntrenador(docSnap.data().nombre || docSnap.data().representante || "Entrenador");
        }
  ...
  const navItems = [
    { icon: "home", screen: "PanelEntrenador" },
          ...
          >
            ...
            <View style={styles.modalHeader}>
              ...
              <View style={{ marginLeft: 12, flex: 1 }}>
                <Text style={[styles.modalNombre, { color: tema.texto }]}>{nombreEntrenador}</Text>
                <Text style={[styles.modalRol, { color: tema.subtexto }]}>Entrenador</Text>
                <Text style={[styles.modalEmail, { color: tema.subtexto }]} numberOfLines={1}>{emailEntrenador}</Text>
              </View>
            </View>
            ...
            {/* TEMA */}
            <TouchableOpacity style={styles.modalOpcion} onPress={toggleTema}>
              <Icon
                name={modoOscuro ? "weather-night" : "white-balance-sunny"}
                size={22}
                color={tema.acento}
              />
              <Text style={[styles.modalOpcionTexto, { color: tema.texto, flex: 1 }]}>
            ...
            <TouchableOpacity style={styles.modalOpcion} onPress={handleLogout}>
              ...
              <Text style={[styles.modalOpcionTexto, { color: "#E24B4A", flex: 1 }]}>Cerrar Sesión</Text>
              <Icon name="chevron-right" size={20} color={tema.subtexto} />
            </TouchableOpacity>
          </View>
        </TouchableOpacity>
      </Modal>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.header, { backgroundColor: tema.inputBg }]}>
          ...
          <View style={styles.headerTexto}>
            ...
            <Text style={[styles.nombreText, { color: tema.texto }]} numberOfLines={1}>{nombreEntrenador}</Text>
          </View>
        ...
        {/* TÍTULO */}
        <Text style={[styles.panelTitle, { color: tema.texto }]}>Panel Entrenador</Text>
        ...
        <View style={styles.grid}>
          {accesosRapidos.map((item) => (
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate(item.screen, { rol: "entrenador" })}
            >
      ...
      <View style={[styles.bottomNav, { backgroundColor: tema.inputBg, borderTopColor: tema.inputBorder }]}>
        {navItems.map((item) => (
          <TouchableOpacity
            key={item.screen}
            onPress={() => {
              setTabActiva(item.screen);
              navigation.navigate(item.screen, { rol: "entrenador" });
            }}
          >
            <Icon name={item.icon} size={28} color={tabActiva === item.screen ? tema.acento : tema.icono} />
          </TouchableOpacity>
        ))}
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  modalOpcionTexto: { fontSize: 15, fontWeight: "500" },
